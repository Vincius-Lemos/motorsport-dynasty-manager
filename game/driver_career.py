...
from .models import Driver, Team, Season, SeasonRound, RaceResult
...
class DriverCareer:
    ...
    def simulate_next_race(self, player_strategy=None) -> Tuple[list, list]:
        ...
        # Salário: fração semanal do salário anual vai para o jogador
        if not self.player_driver.is_injured:
            weekly = self.player_driver.salary // max(1, len(self.season.rounds))
            self.profile.receive_salary(weekly)
    ...
    def start_new_season(self, promote: bool, new_team_id: Optional[str] = None):
        ...
        # Determina equipe
        target_team_id = new_team_id or (
            self.current_team.id if self.current_team else self.all_teams[0].id)
