...
def simulate_race(
    ...
    for st in states:
        pts = 0
        if not st.dnf and st.position <= len(active_scoring):
            pts = active_scoring[st.position - 1]
        is_fl = (fastest_lap_bonus and fl_state and
                 st.driver.id == fl_state.driver.id and st.position <= 10)
        if is_fl:
            pts += 1
        ...
        results.append(RaceResult(
            ...
            points=pts,
            best_lap_time=st.fastest_lap_time if st.fastest_lap_time < float("inf") else 0.0,
        ))

    # Fill gap_to_ahead
    fin = [r for r in results if not r.dnf]
    fin.sort(key=lambda r: r.position)
    for i, r in enumerate(fin):
        if i == 0:
            r.gap_to_ahead = 0.0
        else:
            r.gap_to_ahead = r.total_time - fin[i - 1].total_time
