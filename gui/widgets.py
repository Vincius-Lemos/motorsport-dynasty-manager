...
def accent_strip(surf, rect, color=T.ACCENT, radius=12):
    ...
    strip = pygame.Surface((6, rect.height), pygame.SRCALPHA)
    strip.fill(color)
    # cantos arredondados na esquerda
    surf.blit(strip, (rect.x, rect.y))
...
def stat_bar(surf, x, y, w, label, value, maxv=99, color=T.ACCENT, fonts=None, anim=1.0):
    ...
    by = y + 18
    pygame.draw.rect(surf, T.BG_INPUT, (x, by, w, 7), border_radius=4)
    frac = max(0.0, min(1.0, value / maxv)) * max(0.0, min(1.0, anim))
    if frac > 0.002:
        c = T.GOLD if value >= 80 else color
        pygame.draw.rect(surf, c, (x, by, max(4, int(w * frac)), 7), border_radius=4)
    return by + 7
